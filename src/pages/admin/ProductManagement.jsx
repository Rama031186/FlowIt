    ...
    return (
      <>
        <div className="d-flex align-items-center gap-3 mb-4">
          ...
          <span className={`status-badge ${p.status.toLowerCase()} ms-2`}>{p.status}</span>
        </div>
                ...
                {p.modules.map(mod => (
                  <div key={mod.id} className="d-flex align-items-center gap-3 p-3 mb-2" style={{ background: 'rgba(25,43,55,0.02)', borderRadius: 12 }}>
                    ...
                    <div className="flex-fill">
                      ...
                      <div style={{ fontSize: 12, opacity: 0.5 }}>{mod.description}</div>
                    </div>
                    <div className="d-flex gap-1">
                      {mod.intensities.map((int, i) => (
                        <span key={int} style={{ fontSize: 10, padding: '3px 8px', borderRadius: 6, background: 'rgba(25,43,55,0.04)', fontWeight: 500 }}>
                          {int}: ${mod.prices[i]}
                        </span>
                      ))}
                    </div>
                    <button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 6 }}><FiEdit2 size={14} style={{ opacity: 0.4 }} /></button>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="content-card">
              ...
              <div className="card-body-custom">
                {[
                  ...
                  { version: '1.0', date: '2025-06-01', change: 'Initial release' },
                ].map((v, i) => (
                  <div key={i} className="d-flex gap-3 mb-3">
                    <div style={{ width: 2, background: i === 0 ? '#ff5640' : 'rgba(25,43,55,0.08)', borderRadius: 1, flexShrink: 0 }} />
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 13 }}>v{v.version}</div>
                      <div style={{ fontSize: 11, opacity: 0.5 }}>{v.date} — {v.change}</div>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <tr key={p.id}>
                  <td>
                    <div className="d-flex align-items-center gap-3">
                      <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(88,153,196,0.1)', color: '#5899c4', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <FiLayers size={16} />
                      </div>
